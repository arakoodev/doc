...
export default function BlogPostItemHeaderTitle({className}) {
  ...
  return (
    <TitleHeading className={clsx(styles.title, className)} itemProp="headline">
      ...
      {metadata.frontMatter.talking && <h2 className='text-base font-normal'>In conversation with {metadata.frontMatter.talking}</h2> }
      {isBlogPostPage ? (
        title
      ) : (
      ...
      {metadata.frontMatter.descriptions &&
        <p className='text-xl font-normal pt-8'>{metadata.frontMatter.descriptions}</p>
      }
      <div className='flex flex-col lg:flex-row py-4 gap-8 mb-32'>
